...
torchrun --standalone --nnodes 1 --nproc-per-node 4 --master_port=12363 train.py \
 batch_size=8 eval_batch_size=8 trainer=apo_trainer\
  dataset=balance_apo_dataset eval_dataset=balance_apo_eval_dataset\
  ...
  exp_name=apo_train_${task_name} dataset.kwargs.previous_K=10 loss.kl_max=5 loss.kl_min=-5\
